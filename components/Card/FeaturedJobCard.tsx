  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <span className="text-primary  text-sm font-semibold border border-primary/20 bg-blue-50/50 px-3 py-1">
          {job.type}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mt-auto">
        {job.tags.map((tag) => (
          <span
            key={tag.name}
            className={` font-semibold text-xs px-4 py-1.5 rounded-full ${tag.color}`}
          >
            {tag.name}
          </span>
        ))}

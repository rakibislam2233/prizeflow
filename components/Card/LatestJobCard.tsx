    ...
    >
      ...
      <div className="flex flex-col flex-1">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span className=" font-semibold text-xs px-4 py-1.5 rounded-full text-emerald-500 bg-emerald-50">
            {job.type}
          </span>
          ...
          {job.tags.map((tag) => (
            <span
              key={tag.name}
              className={` font-semibold text-xs px-4 py-1.5 rounded-full border ${tag.color}`}
            >
              {tag.name}
            </span>
          ))}
